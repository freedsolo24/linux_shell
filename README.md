...
# "${#arr[@]}"   拿数组有多少个元素
---
# 在 Bash 的算术上下文中(( ... )) 用做条件判断 或 $(( ... )) 用做取表达式的值，变量名直接写即可，不需要加 $ 或 ${}, let 等同于 (( )) ; arr[index] index前不需要加 $ 或 ${}
# $((nf - i))
# ((nf - i))
# (( x = nf - i ))
---
# for ((j=len-3; j>=0; j-3))          错误的写法
# for ((j = len - 3; j > 0; j -= 3))  正确的写法
```
## 001. 打印文本文件的行数
...
输入正整数n，打印边长为n的等腰三角形
* 思路
1. 调用函数传参n，在函数里面用$1接参
2. 通过类C的for来进行循环
3. 变量i控制的是行数，每一行有两个变量j控制前导空格输出，k控制*+空格输出
    *        ← 第1行：4个前导空格 + 1个*+空格
   * *       ← 第2行：3个前导空格 + 2个*+空格
  * * *      ← 第3行：2个前导空格 + 3个*+空格
 * * * *     ← 第4行：1个前导空格 + 4个*+空格
* * * * *    ← 第5行：0个前导空格 + 5个*+空格
4. 规律:
(1) 前导空格	n - i 个空格	让星星向右对齐形成等腰
(2) 星号部分	i 个 *，每个后跟一个空格	共 i 个 * 形成等腰结构
(3) 每一行打印完*空格, 剩下空格输出自动补齐,不需要自己打
### 020. 取一行打印仅有一个数字的行
* 思路
1. while true读取每一行
2. 遍历本行的每一个字符, 匹配[0-9], count++
3. 打印count=1的行
### 021. 格式化打印1234, 打印成1,234
* 思路
1. 取一行求len, 小于3的直接输出
2. 从后往前取3个,存下来
3. 从前往后取甩3个,拼接一个逗号, 拼接第二步存下来的
### 022. 处理文本
```bash
111:13443
222:13211
...
```
* 思路
1. while true读取每一行, IFS设置为:, 读入的每一行读到数组中
